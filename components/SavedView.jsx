import { IcoBookmark, IcoTrash } from "./Icons";

export default function SavedView({ savedSearches, setSavedSearches, onRun }) {
  return (
    <div className="sv">
      <div style={{ marginBottom: 4 }}>
        <h2 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: "#e0e4f0" }}>Saved Searches</h2>
        <p style={{ fontSize: 12, color: "#3d4258", marginTop: 2 }}>Re-run any saved query instantly.</p>
      </div>

      {savedSearches.length === 0 && (
        <div className="emp">
          <div style={{ fontSize: 32 }}>🔖</div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 15, color: "#6b7490" }}>No saved searches</div>
          <div style={{ fontSize: 12 }}>Type in the top search bar and click "Save Search".</div>
        </div>
      )}

      {savedSearches.map(s => (
        <div key={s.id} className="scard">
          <IcoBookmark />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500, color: "#e0e4f0", fontSize: 13 }}>{s.name}</div>
            <div style={{ fontSize: 11, color: "#3d4258", marginTop: 2 }}>
              {s.q ? <>&ldquo;{s.q}&rdquo;</> : "All companies"} · {new Date(s.savedAt).toLocaleDateString()}
            </div>
          </div>
          <button className="btn btn-primary btn-sm" onClick={() => onRun(s)}>Run</button>
          <button className="btn btn-ghost btn-sm" onClick={() => setSavedSearches(p => p.filter(x => x.id !== s.id))}>
            <IcoTrash />
          </button>
        </div>
      ))}
    </div>
  );
}
